...
class Settings {
  ...
  <?php 
    ...
    public function register_settings() : void {
        ...
        register_setting( 'renewai_ig1_settings', 'renewai_ig1_openai_model', [
            'type'              => 'string',
            'sanitize_callback' => [$this, 'sanitize_openai_model'],
            'default'           => 'gpt-3.5-turbo',
        ] );
        // Add newsletter field registration with sanitize callback
    ...
    <div class="renewai-ig1-settings-section">
      ...
      <table class="form-table">
        <tr>
          <th scope="row">
            <label for="renewai_ig1_newsletter_optin"><?php 
        ...
        <tr>
          <th scope="row">
            <label for="renewai_ig1_newsletter_email"><?php 
        esc_html_e( 'Email Address', 'renewai-featured-image-generator' );
        ?></label>
          </th>
          <td>
            <input type="email" id="renewai_ig1_newsletter_email" name="renewai_ig1_newsletter_email" value="<?php 
...
<?php 
    ...
    public function process_newsletter_settings( $value ) {
        ...
        if ( !$this->has_completed_first_interaction() ) {
            ...
            if ( isset( $_POST['renewai_ig1_newsletter_optin'] ) && $_POST['renewai_ig1_newsletter_optin'] == '1' ) {
                $email = sanitize_email( wp_unslash( $_POST['renewai_ig1_newsletter_email'] ?? '' ) );
                $first_name = sanitize_text_field( wp_unslash( $_POST['renewai_ig1_newsletter_fname'] ?? '' ) );
                $last_name = sanitize_text_field( wp_unslash( $_POST['renewai_ig1_newsletter_lname'] ?? '' ) );
                if ( $email ) {
     ...
     */
    public function sanitize_fal_model( $value ) : string {
        $allowed_values = ['flux_dev', 'flux_pro_1_1', 'flux_pro'];
        // Return default if value is not in allowed list
        return ( in_array( $value, $allowed_values, true ) ? $value : 'flux_dev' );
    }
    ...
    public function sanitize_openai_model( $value ) : string {
        ...
        // Return default if value is not in allowed list
        return ( in_array( $value, $allowed_values, true ) ? $value : 'gpt-3.5-turbo' );
    }

    /**
     * Sanitize the image generator setting.
     *
     * @param string $value The submitted value
     * @return string The sanitized value
     */
    public function sanitize_image_generator( $value ) : string {
        // Get available generators - this allows for future expansion
        $allowed_values = apply_filters( 'renewai_ig1_available_generators', ['dalle', 'flux'] );
        // Return default if value is not in allowed list
        return ( in_array( $value, $allowed_values, true ) ? $value : renewai_ig1_get_default_generator() );
    }
